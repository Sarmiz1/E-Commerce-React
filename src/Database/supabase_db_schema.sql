 ...
 * EXECUTION ORDER:
 * 1. Extensions
 * 2. Profiles -> Categories -> Addresses
 * 3. Products -> Variants -> Images -> Metrics -> Reviews
 * 4. Carts
 * 5. Orders -> Order Items -> Payments -> Transactions
 * 6. Coupons -> Coupon Redemptions
 * 7. Shipments -> Tracking
 * 8. Inventory Reservations
 * 9. Indexes
 * 10. PostgreSQL Triggers (Safety Constraints)
 * 11. RPCs (checkout, webhooks)
 */
...
CREATE INDEX IF NOT EXISTS idx_inventory_variant_active 
  ON inventory_reservations(variant_id) WHERE status = 'reserved';

-- ==============================================================================
-- 20. TRIGGERS (STOCK SAFETY)
-- ==============================================================================
...
DECLARE
  ...
  v_reserved INTEGER;
BEGIN
  -- Lock variant row (prevents race conditions)
  SELECT stock_quantity INTO v_stock FROM product_variants WHERE id = NEW.variant_id FOR UPDATE;

  -- Cleanup expired reservations implicitly
  UPDATE inventory_reservations SET status = 'released' 
  WHERE variant_id = NEW.variant_id AND status = 'reserved' AND expires_at <= now();

  -- Recalculate active reservations
  SELECT COALESCE(SUM(quantity), 0) INTO v_reserved FROM inventory_reservations
  WHERE variant_id = NEW.variant_id AND status = 'reserved';
...
DROP TRIGGER IF EXISTS ensure_stock ON inventory_reservations;
...
BEGIN
  ...
  -- Process cart
  FOR item IN
    SELECT ci.*, p.name, p.price_cents, pv.price_cents AS variant_price FROM cart_items ci JOIN products p ON p.id = ci.product_id LEFT JOIN product_variants pv ON pv.id = ci.variant_id WHERE ci.cart_id = p_cart_id
  LOOP
    DECLARE
    ...
    BEGIN
      ...
      -- snapshot item
      INSERT INTO order_items (order_id, product_id, variant_id, product_name, price_cents, quantity, total_cents)
  ...
  UPDATE orders SET subtotal_cents = v_total, discount_cents = v_discount, shipping_cents = v_shipping, total_cents = (v_total - v_discount + v_shipping) WHERE id = v_order_id;
...
BEGIN
  ...
  INSERT INTO payments (order_id, provider, provider_reference, status, amount_cents, paid_at)
  SELECT p_order_id, 'stripe', p_reference, 'success', total_cents, now() FROM orders WHERE id = p_order_id;
...
BEGIN
  ...
  INSERT INTO payments (order_id, provider, provider_reference, status, amount_cents)
  SELECT p_order_id, 'stripe', p_reference, 'failed', total_cents FROM orders WHERE id = p_order_id;
