...
export function CartItemRow({ item, onUpdateQty, onRemove, removing, updating }) {
  ...
  return (
    <motion.div
      ...
      className={`flex gap-4 rounded-2xl border border-gray-100 dark:border-white/10 bg-white dark:bg-white/5 p-4 transition-all duration-200 hover:border-indigo-100 hover:shadow-sm ${busy ? "pointer-events-none opacity-50" : ""}`}
    >
      <div className="h-20 w-20 flex-shrink-0 overflow-hidden rounded-xl bg-gray-100 dark:bg-white/10">
        {image ? (
          <img
            src={image}
            alt={name}
          ...
          <div className="flex h-full w-full items-center justify-center text-xs font-black uppercase tracking-widest text-gray-300 dark:text-gray-600">Item</div>
        )}
      </div>
      ...
      <motion.button
        ...
        className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full bg-gray-50 dark:bg-white/5 text-gray-300 dark:text-gray-600 transition-all duration-200 hover:bg-red-50 hover:text-red-500"
