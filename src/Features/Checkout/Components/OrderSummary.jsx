    ...
    <div className="flex items-center gap-3">
      <div className="h-10 w-10 flex-shrink-0 overflow-hidden rounded-xl bg-gray-100">
        {image ? (
          ...
          <div className="flex h-full w-full items-center justify-center text-[9px] font-black uppercase tracking-wider text-gray-300">Item</div>
        )}
      </div>
      <div className="min-w-0 flex-1">
        <p className="line-clamp-1 text-xs font-semibold text-gray-700">{name}</p>
        <p className="text-[10px] text-gray-400">x{getCartItemQuantity(item)}</p>
      </div>
      <p className="flex-shrink-0 text-xs font-black text-gray-900">{formatMoneyCents(getCartItemLineTotal(item))}</p>
    </div>
  );
}
...
function ShippingSelector({ shippingOptions, selectedShipping, onShippingChange }) {
  return (
    <div className="mb-5">
      <p className="mb-2 text-[10px] font-black uppercase tracking-widest text-gray-400">Shipping Method</p>
      <div className="space-y-2">
        {shippingOptions.map((option) => (
          <label
            key={option.id}
            className={`flex cursor-pointer items-center gap-3 rounded-xl border p-3 transition-all duration-150 ${
              selectedShipping === option.id ? "border-indigo-300 bg-indigo-50" : "border-gray-100 hover:border-gray-200"
            }`}
          >
            ...
            <span className="text-sm font-black text-gray-900">{option.price === 0 ? "Free" : formatMoneyCents(option.price)}</span>
          </label>
        ))}
      </div>
    </div>
...
function PriceBreakdown({ totals, coupon }) {
  const rows = [
    ...
    { label: `Tax (${(TAX_RATE * 100).toFixed(1)}%)`, value: totals.tax, style: "text-gray-400" },
  ];
  ...
  return (
    <div className="mt-5 flex flex-col gap-2">
      {[
        ...
        <div key={badge.text} className="flex items-center gap-2 text-[11px] text-gray-400">
...
export function OrderSummary({ cart, coupon, shippingOptions, selectedShipping, onShippingChange, step }) {
  ...
  return (
    <>
      <div className="sticky top-24 hidden rounded-3xl border border-gray-100 bg-white p-6 shadow-sm lg:block">
        <SummaryContent
          cart={cart}
          coupon={coupon}
      ...
      <div className="lg:hidden">
        <div className="fixed bottom-0 left-0 right-0 z-50 border-t border-gray-200 bg-white/95 px-4 py-3 shadow-2xl backdrop-blur-xl">
          <div className="mx-auto flex max-w-lg items-center gap-3">
            ...
            <div className="flex-shrink-0 text-right">
              ...
              <p className="text-lg font-black leading-tight text-gray-900">{formatMoneyCents(totals.total)}</p>
            </div>
          </div>
        </div>
          ...
          {mobileOpen && (
            <>
              ...
              <motion.div
                ...
                className="fixed bottom-0 left-0 right-0 z-50 max-h-[80vh] overflow-y-auto rounded-t-3xl bg-white pb-24 shadow-2xl"
              >
                <div className="p-5">
