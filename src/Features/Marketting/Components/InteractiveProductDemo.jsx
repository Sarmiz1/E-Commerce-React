...
export default function InteractiveProductDemo({ dark }) {
  ...
  const muted = useDark ? "text-white/55" : "text-slate-500";

  return (
    <section className={`px-6 py-20 ${useDark ? "bg-[#08080A]" : "bg-slate-50"}`}>
      <div className="mx-auto grid max-w-6xl gap-8 lg:grid-cols-[0.85fr_1.15fr] lg:items-center">
        ...
        >
          ...
          <h2 className={`text-4xl font-black tracking-tight md:text-5xl ${useDark ? "text-white" : "text-slate-950"}`}>
        ...
        >
          <div className={`flex items-center gap-3 rounded-2xl border px-4 py-3 ${useDark ? "border-white/10 bg-black/30" : "border-slate-200 bg-slate-50"}`}>
