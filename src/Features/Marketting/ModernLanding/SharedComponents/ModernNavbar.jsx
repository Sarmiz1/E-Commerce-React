...
import { ShoppingCart, Menu, X, Sparkles, ShoppingBag, ChevronDown } from 'lucide-react';
...
function NavLink({ link, onClick }) {
  ...
  return (
    ...
    >
      <span className={`${isActive ? 'text-blue-600 dark:text-blue-400' : 'text-gray-600 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400'}`}>
        {link.label}
      </span>
      <motion.span
        className="absolute -bottom-1 left-0 h-0.5 bg-blue-600 rounded-full"
...
export default function ModernNavbar({ navLinks = DEFAULT_LINKS, pageView = 'isNotHome' }) {
  ...
  const closeCart = () => { cartTimer.current = setTimeout(() => setCartOpen(false), 160); };
    ...
    <>
      ...
      >
        ...
        <motion.div
          ...
          style={{ backdropFilter: isScrolled ? 'blur(20px) saturate(180%)' : 'blur(8px)' }}
          ...
          <div className="hidden md:flex items-center gap-7">
            {navLinks.map((link, i) => (
              <motion.div
                ...
                transition={{ delay: 0.08 * i + 0.2, duration: 0.4, ease: EXPO }}
              >
                <NavLink
                  link={link}
                  onClick={link.href.startsWith('#') ? (e) => handleHash(link.href, e) : () => setMobileMenuOpen(false)}
                />
              </motion.div>
            ))}
          </div>
                    ...
                    {cartCount > 0 ? (
                      <>
                        <div className="px-5 py-3.5 border-b border-gray-100 dark:border-white/10 flex items-center justify-between">
                          ...
                          <Link to="/cart" onClick={() => setCartOpen(false)} className="text-xs text-blue-600 hover:underline font-medium">View all</Link>
                        </div>
                          ...
                          {cart.slice(0, 5).map((item, i) => (
                            ...
                            >
                              {(item.thumbnail ?? item.image) && (
                                <img src={item.thumbnail ?? item.image} alt={item.name ?? item.title} className="w-10 h-10 rounded-xl object-cover flex-shrink-0" />
                              )}
                              <div className="flex-1 min-w-0">
                                <p className="text-sm font-medium text-gray-900 dark:text-white truncate">{item.name ?? item.title}</p>
                                <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{item.quantity ?? 1} × ₦{(item.price ?? 0).toLocaleString()}</p>
                              </div>
                            </motion.div>
                          ))}
                        </div>
                        <div className="p-4 bg-gray-50 dark:bg-white/[0.03]">
                          <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }}>
                            <Link to="/cart" onClick={() => setCartOpen(false)} className="block w-full text-center py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl text-sm transition-colors">
                              Checkout →
                            </Link>
                          </motion.div>
                        </div>
                      ...
                      <div className="p-8 text-center">
                        <ShoppingBag size={30} className="mx-auto text-gray-300 dark:text-gray-600 mb-3" />
                        <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">Your cart is empty</p>
                        <Link to="/products" onClick={() => setCartOpen(false)} className="mt-3 inline-block text-sm text-blue-600 font-semibold hover:underline">
                          Start shopping →
                        </Link>
                      </div>
                    )}
              ...
              <div className="dark:bg-black/95 px-6 py-6 flex flex-col gap-5">
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    <Link to="/cart" onClick={() => setMobileMenuOpen(false)} className="relative p-2.5 bg-gray-100 dark:bg-white/10 text-gray-700 dark:text-white rounded-xl flex items-center gap-1.5 text-sm font-bold">
                      <ShoppingCart size={18} />
                      {cartCount > 0 && <span className="text-blue-600">{cartCount}</span>}
                    </Link>
                    <motion.button
                      whileTap={{ scale: 0.95 }}
                      onClick={() => { navigate('/ai-shop'); setMobileMenuOpen(false); }}
