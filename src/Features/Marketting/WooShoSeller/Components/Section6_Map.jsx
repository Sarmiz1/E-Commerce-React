...
// Nigeria SVG map simplified paths (major state outlines)
const CITIES = [
...
export default function Section6_Map() {
    ...
    const ctx = gsap.context(() => {
      ...
      gsap.fromTo('.map-stat',
        ...
        { y: 0, opacity: 1, duration: 0.7, stagger: 0.1, ease: 'expo.out',
          scrollTrigger: { trigger: sectionRef.current, start: 'top 60%' }, delay: 0.5 }
      );
    }, sectionRef);
    return () => ctx.revert();
  }, []);
  ...
  return (
    <div ref={sectionRef} className="w-full min-h-screen flex flex-col items-center justify-center px-6 md:px-16 py-28" style={{ background: '#000' }}>
      ...
      <div className="w-full max-w-5xl flex flex-col lg:flex-row items-center gap-12">
        ...
        <div className="relative flex-shrink-0" style={{ width: 340, height: 420 }}>
          {/* Glow */}
          <div style={{ position: 'absolute', inset: -20, background: 'radial-gradient(ellipse, rgba(16,185,129,0.08) 0%, transparent 70%)', pointerEvents: 'none' }} />
          ...
          <svg viewBox="0 0 340 420" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ width: '100%', height: '100%' }}>
            ...
            <path
              ...
              strokeWidth="1.5"
            />
            {/* Internal state lines suggestion */}
            <path d="M100 120 L260 120 M80 200 L290 200 M60 280 L280 280 M170 40 L170 400" stroke="rgba(16,185,129,0.06)" strokeWidth="1" strokeDasharray="4 6" />

            {/* City dots */}
            {CITIES.map((city, i) => (
              <g key={i} className="map-city" style={{ transformOrigin: `${city.cx}px ${city.cy}px` }}>
                {/* Pulse ring */}
            ...
            {CITIES.slice(1).map((city, i) => (
              <line key={i} x1={CITIES[0].cx} y1={CITIES[0].cy} x2={city.cx} y2={city.cy}
                stroke="rgba(16,185,129,0.12)" strokeWidth="1" strokeDasharray="3 5" />
            ))}
          </svg>
        </div>
        ...
        <div className="flex-1 w-full">
          <div style={{ marginBottom: 24 }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: '#4b5563', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 12 }}>Top seller cities</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {CITIES.slice(0, 5).map((city, i) => (
                <div key={i} className="map-city" style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', borderRadius: 14, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            {[
              ...
              <div key={i} className="map-stat" style={{ padding: '16px', borderRadius: 16, background: `${s.color}0c`, border: `1px solid ${s.color}20` }}>
