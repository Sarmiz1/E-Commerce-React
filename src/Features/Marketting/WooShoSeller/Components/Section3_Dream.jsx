...
function Notification({ icon, title, sub, time, color, delay }) {
  return (
    ...
    >
      <div style={{ width: 40, height: 40, borderRadius: 12, background: `${color}22`, border: `1px solid ${color}44`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, flexShrink: 0 }}>
        {icon}
      </div>
...
export default function Section3_Dream() {
  ...
  return (
    <div ref={sectionRef} className="w-full min-h-screen flex flex-col lg:flex-row items-center justify-center gap-16 px-6 md:px-16 py-28" style={{ background: 'linear-gradient(to bottom, #070712, #0a0a0a)' }}>
      {/* Left: Copy */}
      <div className="flex-1 max-w-lg">
        ...
        <h2 className="font-black tracking-tight text-white mb-6" style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)', lineHeight: 1.1 }}>
        ...
        <p style={{ fontSize: 16, color: '#9ca3af', lineHeight: 1.8, marginBottom: 32 }}>
          Adaeze wakes up to new orders, auto-replied DMs, and a fresh payout — all without touching her phone. That's a Woosho seller's day.
        </p>
      ...
      <div className="flex-shrink-0 relative">
        {/* Glow */}
        <div style={{ position: 'absolute', inset: -40, background: 'radial-gradient(ellipse, rgba(99,102,241,0.15) 0%, transparent 70%)', pointerEvents: 'none' }} />

        {/* Phone frame */}
        <div style={{ width: 300, background: '#0d0d14', border: '8px solid #1c1c28', borderRadius: 44, boxShadow: '0 40px 80px rgba(0,0,0,0.6)', overflow: 'hidden', position: 'relative' }}>
          {/* Status bar */}
          ...
          <div style={{ padding: '8px 16px 20px' }}>
            <div style={{ textAlign: 'center', marginBottom: 16 }}>
              <div style={{ fontSize: 11, color: '#6b7280', fontWeight: 600, marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.1em' }}>Woosho Seller</div>
        ...
        <motion.div
          ...
          style={{ position: 'absolute', top: 20, right: -50, background: '#10b981', borderRadius: 14, padding: '8px 14px', fontSize: 12, fontWeight: 800, color: '#fff', whiteSpace: 'nowrap', boxShadow: '0 8px 24px rgba(16,185,129,0.4)' }}
        >
          +₦24,500 💰
        </motion.div>
        <motion.div
          animate={{ y: [0, 8, 0] }} transition={{ duration: 4, repeat: Infinity, ease: 'easeInOut', delay: 1 }}
          style={{ position: 'absolute', bottom: 40, left: -60, background: '#6366f1', borderRadius: 14, padding: '8px 14px', fontSize: 12, fontWeight: 800, color: '#fff', whiteSpace: 'nowrap', boxShadow: '0 8px 24px rgba(99,102,241,0.4)' }}
        >
          AI replied ⚡
        </motion.div>
      </div>
