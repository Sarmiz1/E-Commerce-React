...
export const MILESTONE_IDX = { processing: 2, shipped: 4, delivered: 6, cancelled: 1 };
...
export function computeETA(status, created_at) {
  ...
  const fmt = (d) =>
    new Date(base.getTime() + d * 86_400_000)
...
// ── Auto-poll hook (lives here so both hooks & components can import it) ───────
export function useAutoPoll(status, cb, ms = 30_000) {
  useEffect(() => {
