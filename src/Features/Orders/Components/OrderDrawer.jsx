...
export default function OrderDrawer({
  ...
  return (
    <>
      ...
      >
        <div className="sticky top-0 z-[900] bg-white dark:bg-[#0D1421] pt-28 px-6 py-5 border-b border-gray-100 dark:border-white/10 flex items-start justify-between gap-4">
          <div>
            <p className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-1">
              Order Details
            </p>
            <h2 className="font-black text-gray-900 dark:text-white text-lg">#{getOrderShortId(order, 12)}</h2>
          ...
          <button
            ...
            className="w-10 h-10 rounded-full bg-gray-100 dark:bg-white/5 hover:bg-red-50 hover:text-red-500 flex items-center justify-center flex-shrink-0 mt-1 transition-all duration-200 border border-transparent hover:border-red-200"
          >
            <Icons.Close c="w-4 h-4 text-gray-500 dark:text-gray-400" />
          </button>
        </div>

        <div className="flex gap-0 px-6 border-b border-gray-100 dark:border-white/10">
          {TABS.map((item) => (
            <button
              key={item.id}
              type="button"
              ...
              className={`relative px-5 py-3.5 text-sm font-bold transition-colors ${
                tab === item.id ? "text-indigo-700" : "text-gray-400 hover:text-gray-700 dark:text-gray-200"
              }`}
            >
              {item.label}
              {tab === item.id ? (
        ...
        <div className="sticky bottom-0 bg-white dark:bg-[#0D1421] border-t border-gray-100 dark:border-white/10 px-6 py-4 space-y-3 z-[900]">
          <button
            ...
            onClick={onClose}
            aria-label="Close"
            className="sm:hidden w-full flex items-center justify-center gap-2 py-3 rounded-2xl bg-gray-100 dark:bg-white/5 hover:bg-gray-200 text-gray-600 dark:text-gray-300 font-bold text-sm transition-colors duration-200 border border-gray-200 dark:border-white/10"
          >
