...
export default function FilterSidebar({ filters, setFilters, maxBudget, selectedCategory, setSelectedCategory, matchingCount }) {
  const { colors, isDark } = useTheme();
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <p className="text-[11px] font-bold uppercase tracking-wider mb-3" style={{ color: colors.text.tertiary }}>Category</p>
      ...
      <div className="pt-2">
        <div className="flex items-center justify-between mb-3">
          <p className="text-[11px] font-bold uppercase tracking-wider" style={{ color: colors.text.tertiary }}>Max Price</p>
          <span className="text-sm font-bold" style={{ color: colors.text.primary }}>{formatMoneyCents(filters.budget)}</span>
        </div>
        <div className="relative pt-2">
          <div className="h-[3px] rounded-full w-full" style={{ background: colors.border.default }}>
            <div className="h-full rounded-full transition-colors" style={{ width: `${maxBudget > 0 ? (filters.budget / maxBudget) * 100 : 100}%`, background: `linear-gradient(to right, ${colors.cta.primary}, ${colors.brand?.electricBlue || colors.cta.primary})` }} />
          </div>
          ...
          />
          {/* Thumb indicator */}
          <div className="absolute top-[-5px] w-4 h-4 rounded-full shadow-lg border-2 pointer-events-none transition-colors" style={{ left: `calc(${maxBudget > 0 ? (filters.budget / maxBudget) * 100 : 100}% - 8px)`, background: colors.surface.primary, borderColor: colors.cta.primary }} />
        </div>
      ...
      {/* Min rating */}
      <div>
        <p className="text-[11px] font-bold uppercase tracking-wider mb-3" style={{ color: colors.text.tertiary }}>Min Rating</p>
      ...
      {/* Live result count */}
      <div className="rounded-2xl p-4 text-center mt-4" style={{ background: colors.surface.tertiary, border: `1px solid ${colors.border.subtle}` }}>
