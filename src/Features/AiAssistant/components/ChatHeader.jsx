...
export function ChatHeader({ cartItems, setShowCart, onClose }) {
  return (
    <div style={{ background: `linear-gradient(135deg, ${ACCENT} 0%, #7c3aed 100%)`, padding: "14px 16px", flexShrink: 0 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          ...
          {/* Close button — critical on mobile where panel is fullscreen */}
          <button
            onClick={onClose}
            aria-label="Close assistant"
            style={{ background: "rgba(255,255,255,0.14)", border: "none", borderRadius: 11, width: 34, height: 34, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}
          >
            <X size={16} color="#fff" />
          </button>
        </div>
