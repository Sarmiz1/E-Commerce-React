...
function TestimonialsCarousel() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      const headings = el.querySelectorAll(".se-th"); if (!headings.length) return;
      gsap.fromTo(headings, { y: 35, opacity: 0 }, { y: 0, opacity: 1, stagger: 0.12, duration: 0.85, ease: "power3.out", clearProps: "all", scrollTrigger: { trigger: el, start: "top 83%", once: true } });
    }, 120);
  ...
  return (
    <section ref={sectionRef} id="testimonials" className="relative py-28 overflow-hidden" style={{ background: "linear-gradient(135deg,#f0f4ff 0%,#fafafa 50%,#f5f0ff 100%)" }}>
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-6">
        <p className="se-th text-center text-xs font-bold uppercase tracking-widest text-indigo-500 mb-3">Real Reviews</p>
        <h3 className="se-th text-4xl font-black text-center mb-16 text-gray-900">What Our Customers Say</h3>
        <div className="relative cursor-grab active:cursor-grabbing select-none" onMouseDown={onDS} onMouseMove={onDM} onMouseUp={onDE} onMouseLeave={onDE} onTouchStart={onDS} onTouchMove={onDM} onTouchEnd={onDE}>
          <AnimatePresence mode="wait">
            <motion.div key={current} initial={{ opacity: 0, x: 80, scale: 0.96 }} animate={{ opacity: 1, x: 0, scale: 1 }} exit={{ opacity: 0, x: -80, scale: 0.96 }} transition={{ duration: 0.42, ease: [0.32, 0.72, 0, 1] }} className="bg-white rounded-3xl shadow-2xl p-10 md:p-14 text-center mx-auto max-w-2xl">
