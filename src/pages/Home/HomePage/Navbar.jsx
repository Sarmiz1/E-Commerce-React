...
// ─── Mega Menu Panel ──────────────────────────────────────────────────────────
function MegaMenu({ data, isTop, onNavigate }) {
  if (!data) return null;
  return (
    <motion.div
      ...
      className="absolute top-full left-1/2 -translate-x-1/2 mt-3 w-[700px] max-w-[95vw] rounded-3xl overflow-hidden shadow-2xl shadow-black/15 border border-gray-100/80 z-50"
                  ...
                  <AnimatePresence>
                    {activeMenu === link.label && (
                      <div onMouseEnter={keepMega} onMouseLeave={closeMega}>
                        <MegaMenu data={MEGA_MENU[link.label]} isTop={isTop} onNavigate={(href) => { navigate(href); setActiveMenu(null); }} />
                      </div>
                    )}
                  </AnimatePresence>
                </div>
          ...
          <div className="flex items-center gap-1 ml-auto flex-shrink-0">
            ...
            <div className="relative"
              ...
              <button
                ...
                className={`nb-icon-btn transition-all duration-200 ${
                  isTop ? "text-white/80 hover:text-white hover:bg-white/12" : "text-gray-500 hover:text-gray-900 hover:bg-gray-100"
            ...
            <div className="relative" onMouseEnter={openCart} onMouseLeave={closeCart}>
              <button
                ref={cartRef}
                className={`nb-icon-btn transition-all duration-200 ${
                  isTop ? "text-white hover:bg-white/12" : "text-gray-700 hover:bg-gray-100"
                }`}
                  ...
                  {!searchQuery && (
                    <div className="mt-4 pt-4 border-t border-gray-50">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {["👜 Bags", "⌚ Watches", "🧣 Scarves", "🕶️ Sunglasses", "👠 Shoes", "🧴 Perfumes", "🏀 Sports", "📱 Tech"].map((cat) => (
