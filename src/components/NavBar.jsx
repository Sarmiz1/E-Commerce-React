              ...
              {cart.map((item, i) => (
                <motion.div key={item?.id || i}
                  ...
                  <div className="w-14 h-14 rounded-xl overflow-hidden flex-shrink-0 bg-gray-100 dark:bg-white/5">
                    {(item.thumbnail || item.image) && <img src={item.thumbnail || item.image} alt={item.name} className="w-full h-full object-cover" />}
                  </div>
                  {/* Info */}
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="font-black text-indigo-600 dark:text-indigo-400 text-sm mt-0.5">{formatMoney((item.price ?? 0) * item.quantity)}</p>
                  </div>
                  {/* Remove */}
