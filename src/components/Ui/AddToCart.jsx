...
const VARIANT_STYLES = {
  primary: {
    ...
    success: "bg-gradient-to-r from-emerald-500 to-teal-600 text-white shadow-emerald-500/25",
  },
  outline: {
  ...
  icon: {
    ...
    success: "w-9 h-9 bg-gradient-to-br from-emerald-500 to-teal-600 text-white",
  },
  pill: {
    ...
    loading: "bg-gray-100 text-gray-400 cursor-not-allowed border border-gray-200",
 ...
 */
function AddToCart({ productId, variant = "primary", quantity = 1, className = "" }) {
  // useAddToCart handles: API call, cart sync, fly animation, state management
  const { handleAdd, loading, success, error } = useAddToCart(productId, { quantity });

  const styles = VARIANT_STYLES[variant] ?? VARIANT_STYLES.primary;
  const state  = loading ? "loading" : success ? "success" : "idle";

  return (
    <div className={variant === "primary" || variant === "outline" || variant === "ghost" ? "space-y-2" : ""}>
      <motion.button
        ...
        aria-label={success ? "Added to cart" : loading ? "Adding to cart" : "Add to cart"}
      >
